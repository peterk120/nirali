...
export async function signToken(email: string, role: 'user' | 'admin' | 'sales', id: string): Promise<string> {
...
export function validateCredentials(email: string, password: string): { isValid: boolean; role: 'user' | 'admin' | 'sales' | null } {
  ...
  if (email === 'sales@gmail.com' && password === '123') {
    return { isValid: true, role: 'sales' };
  }
