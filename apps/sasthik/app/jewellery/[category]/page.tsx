...
const filters = [
  ...
  { name: 'Style', options: ['Traditional', 'Contemporary', 'Minimalist', 'Bridal', 'Temple'] },
  { name: 'Plating', options: ['Gold Plated', 'Silver Plated', 'Rose Gold Plated', 'Rhodium'] },
];
...
export default function CategoryPage({ params }: { params: { category: string } }) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await getProducts({ category: params.category });
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-teal-light/30 border-b border-teal-light py-4">
        <div className="container mx-auto px-6 text-[11px] font-body text-gray-500 tracking-widest uppercase flex gap-2">
          <span>Home</span> / <span>Jewellery</span> / <span className="text-brand-teal font-bold">{categoryName}</span>
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 py-12">
        <div className="flex flex-col md:flex-row gap-12">
          ...
          <aside className="hidden md:block w-64 shrink-0">
            <h2 className="font-heading text-2xl text-brand-dark mb-8 border-b border-teal-light pb-4">Filters</h2>
                  ...
                  <div className="flex flex-col gap-3">
                    {filter.options.map((opt) => (
                      <label key={opt} className="flex items-center gap-3 cursor-pointer group">
                        ...
                        <span className="text-[13px] text-gray-600 group-hover:text-brand-teal transition-colors font-body">{opt}</span>
                      </label>
                    ))}
                  </div>
                </div>
          ...
          <main className="flex-grow">
            <div className="flex justify-between items-end mb-10 pb-6 border-b border-teal-light">
              <div>
                <h1 className="font-heading text-4xl md:text-5xl text-brand-dark mb-2">{categoryName}</h1>
                <p className="text-[12px] text-gray-400 font-body tracking-wider uppercase">{products.length} Results found</p>
              </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-20 bg-teal-light/20 rounded-3xl border border-dashed border-teal-light">
                <p className="font-body text-gray-500 uppercase tracking-widest text-xs">No products found in this category.</p>
              </div>
            ) : (
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-10">
                <h2 className="font-heading text-3xl">Filters</h2>
                    ...
                    <div className="flex flex-col gap-4">
                      {filter.options.map((opt) => (
                        <label key={opt} className="flex items-center gap-4 cursor-pointer">
                          ...
                          <span className="text-[14px] text-gray-600 font-body">{opt}</span>
                        </label>
                      ))}
                    </div>
                  </div>
              ...
              <button className="w-full bg-brand-teal text-white py-4 mt-12 rounded-lg font-bold tracking-widest uppercase text-xs">Apply Filters</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
